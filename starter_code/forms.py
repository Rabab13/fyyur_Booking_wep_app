...
# format validation phone number 
def validation(form, phone):
    if not re.search(r"^[0-9]*$", form, phone):
...
# added website, seeking venue & seeking description for both venue and artist attribute 
    website = StringField(
...
class ArtistForm(Form):
    ...
    phone = StringField(
        'phone', validators=[DataRequired([validation])]
    )
    image_link = StringField(
        'image_link'
    )
